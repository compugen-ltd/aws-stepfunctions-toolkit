...
def _create_map_state_output(history: ExecutionHistory, state_name: str) -> AllOutput:
    ...
    for event in history:
        ...
        if (
            ...
            and event["mapIterationSucceededEventDetails"]["name"] == state_name
            and i > 0
        ):
...
def _create_task_failed_output(failed_event: HistoryEventTypeDef) -> AllOutput:
    ...
    return {
        "mock": {"errorOutput": {"error": details["error"], "cause": details["cause"]}},
        "type": "task",
    }
...
def _create_step_function_output(output: dict[str, Any]) -> AllOutput:
    ...
    return {"mock": {"result": json.dumps(output)}, "task": False}
...
def _create_generic_task_output(output: dict[str, Any]) -> AllOutput:
    ...
    return {"mock": {"result": json.dumps(output)}, "task": True}


def _process_map_state_event(
    execution_history: ExecutionHistory, history_event: HistoryEventTypeDef
) -> tuple[str, AllOutput]:
...
def _process_task_state_event(
    history: ExecutionHistory, history_event: HistoryEventTypeDef
) -> tuple[None, None] | tuple[str, AllOutput]:
...
def generate_mock_data(
    ...
    execution_response: DescribeExecutionOutputTypeDef = sfn_client.describe_execution(
