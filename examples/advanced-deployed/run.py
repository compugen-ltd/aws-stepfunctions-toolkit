...
import boto3
from aws_stepfunctions_toolkit import WorkflowRunner, CallableStrategy, LocalExecutionStrategy
...
definition = json.loads(sfn.describe_state_machine(stateMachineArn=STATE_MACHINE_ARN)["definition"])


def invoke_real_lambda(input_data: dict):
    resp = lam.invoke(FunctionName=FUNCTION_ARN, Payload=json.dumps(input_data).encode())
    payload = json.loads(resp["Payload"].read())
    return {"Payload": json.dumps(payload)}  # Payload as a JSON string (toolkit convention)


mock_mapping = {
    "Enrich": CallableStrategy(invoke_real_lambda),  # triggers the REAL Lambda
    "ProcessLocally": LocalExecutionStrategy(entrypoint=[sys.executable, str(HERE / "job" / "main.py")]),  # local script
}
